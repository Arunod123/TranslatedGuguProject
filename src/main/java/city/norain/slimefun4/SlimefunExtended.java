...
public final class SlimefunExtended {
    ...
    public static boolean checkEnvironment(@Nonnull Slimefun sf) {
        try {
            ...
            sf.getLogger().log(Level.WARNING, "Could not recognize the serviceend version you are using:");
            return false;
        }
        ...
        if (EnvironmentChecker.checkHybridServer()) {
            ...
            sf.getLogger().log(Level.WARNING, "");
            sf.getLogger().log(Level.WARNING, "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            sf.getLogger().log(Level.WARNING, "Slimefun was detected using a blending end and will be disabled!");
            sf.getLogger().log(Level.WARNING, "The mixed end has been reported by multiple users as having problems with usage.");
            sf.getLogger().log(Level.WARNING, "The mandatory circumvention of testing will not be supported by any feedback.");
            sf.getLogger().log(Level.WARNING, "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            sf.getLogger().log(Level.WARNING, "");
            sf.getLogger().log(Level.WARNING, "#######################################################");
            return false;
        ...
        if (Slimefun.getConfigManager().isBypassEnvironmentCheck()) {
            ...
            sf.getLogger().log(Level.WARNING, "");
            sf.getLogger().log(Level.WARNING, "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            sf.getLogger().log(Level.WARNING, "You've been detected using environmental compatibility tests!");
            sf.getLogger().log(Level.WARNING, "Failure to pass compatibility checks will not be supported by feedback ...");
            sf.getLogger().log(Level.WARNING, "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            sf.getLogger().log(Level.WARNING, "");
            sf.getLogger().log(Level.WARNING, "#######################################################");
            return true;

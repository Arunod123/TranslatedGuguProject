...
public final class WikiUtils {
    ...
    public static void setupJson(Plugin plugin, Function<String, String> formatter) {
        if (!(plugin instanceof SlimefunAddon)) {
            throw new IllegalArgumentException("This plugin is not attached to Slimefun");
        }
        try (BufferedReader reader = new BufferedReader(
            ...
            plugin.getLogger()
                    .log(Level.INFO, MessageFormat.format("_Other Organiser", plugin.getName(), count));
        } catch (Exception e) {
            plugin.getLogger().log(Level.SEVERE, MessageFormat.format("Could not close temporary folder: %s", plugin.getName()), e);
        }
    }
}

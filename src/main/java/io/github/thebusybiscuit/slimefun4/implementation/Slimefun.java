...
public final class Slimefun extends JavaPlugin implements SlimefunAddon, ICompatibleSlimefun {
    ...
    public void onEnable() {
        ...
        if (isUnitTest()) {
            ...
            getLogger().log(Level.WARNING, "");
            getLogger().log(Level.WARNING, "Slimefun Han version since 24/12/22");
            getLogger().log(Level.WARNING, "Convert to PaperSmart plugin, you have to use PaperSmart");
            getLogger().log(Level.WARNING, "Slimefun.");
            getLogger().log(Level.WARNING, "Paper: https://papermc.io/downloads/paper");
            getLogger().log(Level.WARNING, "");
            getLogger().log(Level.WARNING, "#######################################################");
            getServer().getPluginManager().disablePlugin(this);
        } else {
    ...
    private void onPluginStart() {
        ...
        if (PlayerProfileMigrator.getInstance().hasOldData()
            ...
            Slimefun.logger().warning("\n");
            Slimefun.logger().log(Level.WARNING, "Found old player data stored with file!");
            Slimefun.logger().warning("Please use /sf migration control after the server has been loaded!");
            Slimefun.logger().warning("If you don't move, you'll lose the previous version!");
            Slimefun.logger().warning("\n");
            Slimefun.logger().warning("Users who need to use MySQL database, please modify two configuration files after the service is closed");
        ...
        if (networkSize < 1) {
            logger.log(Level.WARNING, "'Networks.max-size' size set error! It must be greater than one, and you set it:", networkSize);
            networkSize = 1;
        }
        ...
        logger.log(Level.INFO, "Load Custom Tabs...");
        loadTags();
        ...
        if (cfgManager.isAutoUpdate()) {
            // Automatically update the Han version
            Bukkit.getScheduler().scheduleSyncDelayedTask(this, new AutoUpdateTask(this, getFile()));
        }

        // Hooray!
        logger.log(Level.INFO, "Slimefun loaded, time-consuming", getStartupTime(timestamp));
    }
    ...
    private boolean isVersionUnsupported() {
        try {
            ...
            if (version > 0) {
                ...
                getLogger().log(Level.WARNING, "Can not get folder: %s: %s", version);

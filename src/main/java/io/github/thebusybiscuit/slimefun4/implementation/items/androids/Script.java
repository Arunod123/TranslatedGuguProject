...
public final class Script {
    ...
    ItemStack getAsItemStack(@Nonnull ProgrammableAndroid android, @Nonnull Player p) {
        ...
        lore.add("& 7 Author & f" + getAuthor());
        lore.add("");
        lore.add("& 7 downloads: & f" + getDownloads());
        lore.add("& 7:" + getScriptRatingPercentage());
        lore.add("&a" + getUpvotes() + " \u263A &7| &4\u2639 " + getDownvotes());
        lore.add("");
        lore.add("& e Left Key & f Download Script");
        lore.add("& 4 (will cover your existing scripts!");

        if (canRate(p)) {
            lore.add("");
            lore.add("& eShift+Left & f");
            lore.add("& eShift+Right Key & f Bad Review");
        }

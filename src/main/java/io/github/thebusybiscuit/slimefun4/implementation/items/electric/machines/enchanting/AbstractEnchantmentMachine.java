...
abstract class AbstractEnchantmentMachine extends AContainer {
    ...
    private final ItemSetting<List<String>> ignoredLores = new ItemSetting<>(
            this, "ignored-lores", Collections.singletonList("& 7- & c cannot be used in" + this.getItemName() + "Let's go."));
    ...
    protected void showEnchantmentLevelWarning(@Nonnull BlockMenu menu) {
        if (!useLevelLimit.getValue()) {
            throw new IllegalStateException("Auto-Accelerator Level Limit is not enabled, warning information cannot be displayed.");
        }
    ...
    protected void showEnchantmentLimitWarning(@Nonnull BlockMenu menu) {
        if (!useEnchantLimit.getValue()) {
            throw new IllegalStateException("The number limit for automatic attachment/cancer attachments is not enabled, and warning messages cannot be displayed.");
        }

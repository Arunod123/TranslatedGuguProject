...
public abstract class AbstractEntityAssembler<T extends Entity> extends SimpleSlimefunItem<BlockTicker>
    ...
    private void updateBlockInventory(BlockMenu menu, Block b) {
        ...
        if (blockData == null || (val = blockData.getData(KEY_ENABLED)) == null || val.equals(String.valueOf(false))) {
            menu.replaceExistingItem(22, new CustomItemStack(Material.GUNPOWDER, "Availability of & 7: & 4\u2718", "", "& e> Click to open machine"));
            ...
            menu.replaceExistingItem(22, new CustomItemStack(Material.REDSTONE, "Availability of & 7: & 2\u2714", "", "& e> Click to close machine"));
            menu.addMenuClickHandler(22, (p, slot, item, action) -> {
        ...
        menu.replaceExistingItem(
                31,
                new CustomItemStack(
                        Material.PISTON, "& 7 Generate height: & 3" + offset + "Square block height", "", "& f Left Key: & 7+0.1", "& f Right Key: & 7-0.1"));
    ...
    protected void constructMenu(BlockMenuPreset preset) {
        preset.addItem(
                1,
                new CustomItemStack(getHead(), "& 7 puts the head here", "", "& f This can be placed as an object generating the head of the entity."),
                ChestMenuUtils.getEmptyClickHandler());
        preset.addItem(
                7,
                new CustomItemStack(getBody(), "& 7 is here to assemble raw materials", "", "& f can be placed here as an object generating the body torso."),
                ChestMenuUtils.getEmptyClickHandler());
        preset.addItem(
                13,
                new CustomItemStack(Material.CLOCK, "& 7 cooling time: & b30 seconds", "", "The machine takes half a minute to assemble.", "So wait and see!"),
                ChestMenuUtils.getEmptyClickHandler());
    }

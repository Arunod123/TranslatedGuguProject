...
public class ProduceCollector extends AContainer implements RecipeDisplayItem {
    ...
    public @Nonnull List<ItemStack> getDisplayRecipes() {
        ...
        displayRecipes.add(new CustomItemStack(Material.BUCKET, null, "& f requires a nearby & b cow & f for production"));
        displayRecipes.add(new ItemStack(Material.MILK_BUCKET));

        if (Slimefun.getMinecraftVersion().isAtLeast(MinecraftVersion.MINECRAFT_1_17)) {
            displayRecipes.add(new CustomItemStack(Material.BUCKET, null, "& f Requires & b goats & f nearby for production"));
            displayRecipes.add(new ItemStack(Material.MILK_BUCKET));
        }

        displayRecipes.add(new CustomItemStack(Material.BOWL, null, "& f needs & brooms & f nearby for production"));

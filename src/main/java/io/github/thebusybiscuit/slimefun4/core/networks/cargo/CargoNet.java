...
public class CargoNet extends AbstractItemNetwork implements HologramOwner {
    ...
    public void tick(@Nonnull Block b, SlimefunBlockData blockData) {
        if (!regulator.equals(b.getLocation())) {
            updateHologram(b, "& 4 found multiple cargo network adjusters nearby", blockData::isPendingRemove);
            return;
        }
        ...
        if (connectorNodes.isEmpty() && terminusNodes.isEmpty()) {
            updateHologram(b, "& c. Can't find a nearby freight network node", blockData::isPendingRemove);
        } else {
            updateHologram(b, "& 7 Status: & a&l connected", blockData::isPendingRemove);

...
class BackpackCommand extends SubCommand {
    ...
    private void showBackpackMenu(OfflinePlayer owner, Player p, Set<PlayerBackpack> result, int page) {
        var menu = new ChestMenu(owner.getName() + "List of owned backpacks");
        ...
        for (int i = 0; i <= 36; i++) {
            ...
            im.setDisplayName(bp.getName().isEmpty() ? "Packet#" + bp.getId() : bp.getName());
            var lore = new ArrayList<String>();
            lore.add("");
            lore.add(ChatColor.translateAlternateColorCodes('&', "& a left key to get this backpack"));
            im.setLore(lore);

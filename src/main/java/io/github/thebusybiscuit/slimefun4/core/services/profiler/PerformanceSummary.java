...
class PerformanceSummary {
    ...
    public void send(@Nonnull PerformanceInspector sender) {
        sender.sendMessage("");
        sender.sendMessage(ChatColor.GREEN + "Slimefun performance analyser");
        sender.sendMessage(
                ChatColor.GOLD + "Tick, all the time:" + ChatColor.YELLOW + NumberUtils.getAsMillis(totalElapsedTime));
        sender.sendMessage(ChatColor.GOLD
                + "Ticker running cycle:"
                + ChatColor.YELLOW
        ...
        summarizeTimings(totalTickedBlocks, "block", sender, items, entry -> {
            ...
            if (sender.getOrderType() == SummaryOrderType.AVERAGE) {
                return String.format(message, average + "Total time spent:" + time);
            } else {
                return String.format(message, time + "Average time spent:" + average);
            }
        });
    ...
    private TextComponent summarizeAsTextComponent(
        ...
        if (count > 0) {
            TextComponent hoverComponent = new TextComponent("(Put the mouse here for more information)");

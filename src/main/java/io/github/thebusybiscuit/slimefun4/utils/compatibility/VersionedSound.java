...
public final class VersionedSound {
    ...
    static {
        ...
        try {
            ...
            Slimefun.logger().severe("Unable to determine the Sound type:" + e.getMessage());
        }
        ...
        try {
            if (isEnum()) {
                ...
                if (!(key instanceof NamespacedKey nsKey)) {
                    throw new IllegalStateException("Sound. GetKey() does not returnNamespacedKey");
                }
                return nsKey.getKey();
            }
        } catch (Exception e) {
            Slimefun.logger().severe("Could not close temporary folder: %s" + e.getMessage());
        }
    ...
    public static Sound valueOf(@Nonnull String name) throws IllegalArgumentException {
        ...
        try {
            ...
            throw new IllegalArgumentException("Could not close temporary folder: %s" + name, e);
        }

        throw new IllegalArgumentException("Sound.valueof method not available");
    }
}

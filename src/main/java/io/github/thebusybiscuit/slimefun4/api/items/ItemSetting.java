...
public class ItemSetting<T> {
    ...
    protected @Nonnull String getErrorMessage() {
        // FIX: Cleaned up the error message
        return "Please use a valid value of type '" + defaultValue.getClass().getSimpleName() + "'!";
    }
    ...
    public void reload() {
        ...
        if (defaultValue.getClass().isInstance(configuredValue)
            ...
            if (validateInput(newValue)) {
                ...
                // @formatter:off
                item.warn("Found invalid item settings in Items.yml!"
                        + "\nOn \"" // FIX: Corrected string concatenation
                        + item.getId()
                        + "."
                        + getKey()
                        + "\""
                        + "\n    \"" // FIX: Added quotes and spacing
                        + configuredValue
                        + "\" is not a valid value!" // FIX: Improved message
                        + "\n"
                        + getErrorMessage());
                // @formatter:on
            }
            ...
            item.warn("Found invalid item settings in Items.yml!"
                    ...
                    + "\" but found: \"" // FIX: Replaced Chinese characters
                    + found
                    + "\"");
            // @formatter:on
        }

...
public class ErrorReport<T extends Throwable> {
    ...
    public ErrorReport(T throwable, Location l, SlimefunItem item) {
        this(throwable, item.getAddon(), stream -> {
            stream.println("Square information:");
            ...
            stream.println("Square data:" + l.getBlock().getBlockData().getClass().getName());
            stream.println("Status:" + l.getBlock().getState().getClass().getName());
            stream.println();

            if (item.getBlockTicker() != null) {
                stream.println("Can not open message");
                stream.println("Type:" + (item.getBlockTicker().isSynchronized() ? "Sync" : "Achilles"));
                stream.println();
            }
                        ...
                        .ifPresentOrElse(
                                uuid -> {
                                    ...
                                    if (universalData != null) {
                                        ...
                                        stream.println("Object column:" + (universalData.getMenu() != null));
                                        stream.println("Data:");
                                        universalData
                                                .getAllData()
                                                .forEach((k, v) -> stream.println("    " + k + ": " + v));
                                    } else {
                                        stream.println("There's no data on the square.");
                                    }
                                },
                                () -> stream.println("There's no data on the square.")));
            } else {
                stream.println("Data loading status:" + blockData.isDataLoaded());
                stream.println("Object column:" + (blockData.getBlockMenu() != null));
                stream.println("Data:");

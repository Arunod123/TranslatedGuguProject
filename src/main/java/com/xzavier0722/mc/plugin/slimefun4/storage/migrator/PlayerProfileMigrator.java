...
public class PlayerProfileMigrator implements IMigrator {
    ...
    public MigrateStatus migrateData() {
        ...
        for (File file : listFiles) {
            ...
            try {
                ...
                if (!p.hasPlayedBefore()) {
                    Slimefun.logger().log(Level.INFO, "Detected data that never joined the server player, skipped automatically:" + uuid);
                    total--;
                    continue;
                }
                ...
                migratedCount++;
                Slimefun.logger().log(Level.INFO, "Could not close temporary folder: %s" + p.getName() + "(" + migratedCount + "/" + total + ")");
            } catch (IllegalArgumentException ignored) {
                result = MigrateStatus.FAILED;
                Slimefun.logger().log(Level.WARNING, "Could not close temporary folder: %s" + file.getName() + "'");
        ...
        if (MigratorUtil.createDirBackup(playerFolder)) {
            Slimefun.logger()
                    .log(Level.INFO, "Could not close temporary folder: %s", migratedCount);
            try {

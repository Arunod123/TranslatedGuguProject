...
public abstract class SqlCommonAdapter<T extends ISqlCommonConfig> implements IDataSourceAdapter<T> {
    ...
    public void patch() {
        ...
        Slimefun.logger().log(Level.INFO, "_Other Organiser", new Object[] {dbVer});
        ...
        try (var conn = ds.getConnection()) {
            Slimefun.logger().log(Level.INFO, "Updating database version to" + patch.getVersion() + "It could take a while...");
            ...
            Slimefun.logger().log(Level.SEVERE, "Problems in updating the database!", e);
        }
    }
}

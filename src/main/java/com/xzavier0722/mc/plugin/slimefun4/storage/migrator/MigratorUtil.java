...
class MigratorUtil {
    protected static boolean createDirBackup(File dir) {
        try {
            ...
            if (Files.exists(backupPath, LinkOption.NOFOLLOW_LINKS)) {
                Slimefun.logger().log(Level.WARNING, "Found backup data already available, skipping backup");
                return true;
            }
                ...
                try (var fs = Files.walk(src).filter(path -> !Files.isDirectory(path))) {
                    fs.forEach(path -> {
                        ...
                        try {
                            ...
                            Slimefun.logger().log(Level.WARNING, "Backup old data" + dir.getName() + "There's a problem.", e);
                        }
                    });
                }
            }
    ...
    protected static void deleteOldFolder(File dir) {
        try {
            ...
            Slimefun.logger().log(Level.WARNING, "Remove Folder" + dir.getAbsolutePath() + "There's a problem.", e);
        }
    }
